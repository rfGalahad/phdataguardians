...
export const useMembers = () => {
  ...
  const [selectedMember, setSelectedMember] = useState(null); // selected member for member details dialog
  const [selectedMemberID, setSelectedMemberID] = useState(null); // selected member ID for print ID dialog
  ...
  const getMemberData = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          Digital_ID(*)
        `)
        .in("Payment.payment_status", ["Confirmed"]);
      ...
      const getMembershipStatus = (expiration) => {
        const exp = expiration ? new Date(expiration) : null;
        if (!exp || isNaN(exp)) return "Unknown";
        return Date.now() > exp.getTime() ? "Expired" : "Active";
      }
      ...
      const formatted = data.map((m) => {
        ...
        return {
          ...
          expirationDate: formatDate(digital?.expiration_date),

          membershipStatus: getMembershipStatus(digital?.expiration_date),
